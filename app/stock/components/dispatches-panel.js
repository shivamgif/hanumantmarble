...
export function DispatchesPanel({
  ...
  return (
    <div className="stock-tab-panel" key="stock-panel-dispatches">
      <section id="dispatches" className="flex h-full flex-col overflow-hidden scroll-mt-6 rounded-xl border border-slate-200/70 bg-white dark:border-slate-700/70 dark:bg-slate-900">
        <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50/60 px-4 py-3 dark:border-slate-800 dark:bg-slate-900/40">
          <h2 className="text-sm font-semibold text-foreground">{t('dispatches')}</h2>
          ...
          <table className="hidden w-full text-xs text-left whitespace-nowrap md:table">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {dispatchPagination.rows.map((d) => (
                <tr
                  key={d.id}
                  className={`cursor-pointer transition hover:bg-slate-50 focus-within:bg-slate-50 dark:hover:bg-slate-800/40 dark:focus-within:bg-slate-800/40 ${highlightedShipmentKey === `dispatch-${d.id}` ? 'bg-primary/10 ring-1 ring-primary/40' : 'odd:bg-white even:bg-slate-50/70 dark:odd:bg-slate-900 dark:even:bg-slate-900/70'}`}
                  onClick={() => openShipmentPreview('dispatch', d)}
                  tabIndex={0}
                  role="button"
                  onKeyDown={(event) => {
