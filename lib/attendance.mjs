...
// Punch selfies. These two live here, in the pure module, because both the
// Next.js runtime (lib/attendance-selfie.mjs) and the standalone nightly purge
// (netlify/functions/purge-selfies.mjs) need them and neither can import the
// other's world.
export const SELFIE_STORE_NAME = 'attendance-selfies';
// Long enough to settle a payroll dispute, short enough that we are not sitting
// on years of staff photographs.
...
/**
 * A boolean that survives the round trip. The driver hands back a real boolean,
 * but the settings PUT merges raw form values, and Boolean('false') is true —
 * which would silently turn a switch ON when a manager turned it off.
 */
...
/**
 * One shape for a shift time, "HH:MM".
 *
 * A TIME column arrives from the driver as "09:30:00"; an <input type="time">
 * sends "09:30". The settings PUT merges the two — it validates the result of
 * `{ ...loadSettings(), ...body }` — so a manager who saved without retyping
 * both shift times sent the seconds straight back and was rejected by the
 * route's HH:MM check. Settling on one shape here fixes every caller at once.
 *
 * Only a well-formed HH:MM:SS is trimmed. Anything else passes through
 * UNCHANGED rather than falling back to a default: these times drive payroll,
 * and the route is meant to reject a typo, not silently replace it with 09:30.
 */
export function clockValue(value, fallback) {
  if (value === null || value === undefined || value === '') return fallback;
...
export function normalizeSettings(settings) {
  const s = settings || {};
  return {
    shift_start: clockValue(s.shift_start, DEFAULT_SETTINGS.shift_start),
    shift_end: clockValue(s.shift_end, DEFAULT_SETTINGS.shift_end),
    ...
    require_selfie: bool(s.require_selfie, DEFAULT_SETTINGS.require_selfie),
  };
}
...
/**
 * The anchored location closest to a GPS fix, as { location, distance }.
 *
 * This is how a punch gets attributed once there is more than one site: the
 * employee never picks a branch, their coordinates do. Locations with no
 * coordinates are skipped — an un-anchored site cannot be "nearest" to
 * anything, and including it would silently swallow punches.
 *
 * Returns null when there is no fix or nothing anchored to compare against.
 * Being far from the nearest site is NOT an error: the punch is attributed
 * there and flagged, which is what a manager needs to see.
 */
...
/**
 * Whether this branch can be geofenced at all — i.e. somebody anchored it to a
 * coordinate. An un-anchored location has nothing to measure against.
 */
export function hasGeofence(location) {
  return coord(location?.latitude) !== null && coord(location?.longitude) !== null;
}

/**
 * Whether a punch must be REFUSED because no position was supplied.
 *
 * Missing coordinates used to be treated as "cannot prove anything" and waved
 * through, which made declining the browser's location prompt the easiest way
 * to punch: one tap on Block and a punch from another continent was recorded
 * with is_outside_geofence = false, indistinguishable from standing in the
 * showroom. A silent bypass is worse than a blocked punch somebody can retry.
 *
 * The rule is narrow on purpose. It bites ONLY when the branch has coordinates
 * to check against — an un-anchored location, or a punch that could not be
 * attributed to any branch, is still recorded, because refusing there would
 * block people over configuration they cannot see or fix.
 *
 * Note this covers a MISSING position, not a distant one. A punch from far away
 * still succeeds and is flagged: a real coordinate is evidence a manager can
 * act on, and there are legitimate reasons to be off-site. An absent one is not
 * evidence of anything.
 */
export function isMissingRequiredPosition(lat, lng, location) {
  if (!hasGeofence(location)) return false;
  return coord(lat) === null || coord(lng) === null;
}

/**
 * Outside the fence only when we can actually prove it. Missing coordinates on
 * either side (location not configured, un-anchored branch) is NOT a violation
 * — those punches are recorded and flagged, never blocked. A punch that is
 * missing a position at an ANCHORED branch never reaches here; see
 * isMissingRequiredPosition above.
 */
