...
/**
 * Where a punch is measured from. Falls back to the seeded 'Showroom' row so a
 * geofence works out of the box once someone fills in its coordinates.
 */
export async function resolvePunchLocation(locationId) {
  const rows = locationId
    ? await sql('SELECT id, name, latitude, longitude FROM stock_locations WHERE id = $1', [locationId])
    : await sql(
        `SELECT id, name, latitude, longitude FROM stock_locations
 ...
 * and non-numeric values before they reach a NUMERIC(9,6) column — and treats
 * a missing reading as missing rather than as 0,0.
 */
