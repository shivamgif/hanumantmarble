...
/** { id, name } for a location, or null. For display only. */
export async function locationSummary(id) {
  if (!id) return null;
  const rows = await sql('SELECT id, name FROM stock_locations WHERE id = $1', [id]);
