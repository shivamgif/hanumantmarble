...
import { decodeSelfie } from './attendance-selfie.mjs';
...
// --- isMissingRequiredPosition ----------------------------------------------
// Declining the browser's location prompt used to be the cheapest way to punch
// from anywhere: no coordinates meant nothing to compare, so the row landed
// unflagged and looked identical to standing in the showroom.
const anchored = { latitude: 26.897788, longitude: 80.967727 };   // Ring Road
const unanchored = { latitude: null, longitude: null };

assert.equal(hasGeofence(anchored), true);
assert.equal(hasGeofence(unanchored), false);
assert.equal(hasGeofence(null), false);
assert.equal(hasGeofence({}), false);

// The bypass, closed: an anchored branch and no fix is refused.
assert.equal(isMissingRequiredPosition(null, null, anchored), true);
assert.equal(isMissingRequiredPosition(26.897788, null, anchored), true, 'half a fix is no fix');
assert.equal(isMissingRequiredPosition(null, 80.967727, anchored), true);
assert.equal(isMissingRequiredPosition('', '', anchored), true, 'empty strings are not coordinates');

// A real fix is always allowed through, however far away — being off-site is
// flagged for a manager, not blocked, and there are legitimate reasons for it.
assert.equal(isMissingRequiredPosition(26.897788, 80.967727, anchored), false);
assert.equal(isMissingRequiredPosition(52.52, 13.405, anchored), false, 'Berlin is recorded, then flagged');
assert.equal(isOutsideGeofence(52.52, 13.405, anchored, cfg), true, '...and it IS flagged');

// 0,0 is a real coordinate. It must count as a fix, not read as absent.
assert.equal(isMissingRequiredPosition(0, 0, anchored), false);

// Nothing to check against means nobody is blocked over configuration they
// cannot see: an un-anchored branch, or a punch that matched no branch at all.
assert.equal(isMissingRequiredPosition(null, null, unanchored), false);
assert.equal(isMissingRequiredPosition(null, null, null), false);
assert.equal(isMissingRequiredPosition(null, null, undefined), false);

// --- shift times ------------------------------------------------------------
// A TIME column arrives as "09:30:00" and an <input type="time"> sends "09:30".
// The settings PUT validates the merge of the two against an anchored HH:MM
// regex, so saving without retyping the times used to 400.
const TIME_RE = /^([01]\d|2[0-3]):[0-5]\d$/;
assert.equal(clockValue('09:30:00', '09:30'), '09:30', 'driver seconds are trimmed');
assert.equal(clockValue('09:30', '09:30'), '09:30', 'an input value passes through');
assert.equal(clockValue('', '09:30'), '09:30', 'missing falls back');
assert.equal(clockValue(null, '19:00'), '19:00');
assert.equal(clockValue(undefined, '19:00'), '19:00');
// A typo must NOT be swallowed — these drive payroll, and the route is meant to
// reject it rather than quietly substitute a default.
assert.equal(clockValue('25:99', '09:30'), '25:99');
assert.equal(clockValue('nonsense', '09:30'), 'nonsense');
assert.equal(TIME_RE.test(clockValue('25:99', '09:30')), false, 'a bad time still fails validation');

// The regression itself: a settings row straight from the driver, merged with a
// form body that did not touch the times, must survive the route's check.
const merged = normalizeSettings({
  ...normalizeSettings({ shift_start: '09:30:00', shift_end: '19:00:00' }),
  require_selfie: true,
});
assert.ok(TIME_RE.test(merged.shift_start), 'shift_start survives the round trip');
assert.ok(TIME_RE.test(merged.shift_end), 'shift_end survives the round trip');
assert.equal(merged.require_selfie, true);
// Normalising twice must not drift.
assert.deepEqual(normalizeSettings(merged), merged, 'normalizeSettings is idempotent');
// parseClock already tolerated seconds, so late marks were never wrong — pin it.
assert.equal(classifyDay(480, '2026-09-01T09:50:00', { ...cfg, shift_start: '09:30:00' }).lateMinutes, 5);

// --- selfie settings --------------------------------------------------------
// Boolean('false') is true, and the settings PUT merges raw form values — a
// manager unticking the box must not silently leave it on.
assert.equal(bool('false'), false);
assert.equal(bool('0'), false);
assert.equal(bool('true'), true);
assert.equal(bool('on'), true);
assert.equal(bool(undefined, true), true, 'a missing value keeps the current setting');
assert.equal(bool(''), false);
assert.equal(normalizeSettings({}).require_selfie, false, 'selfies are off unless turned on');
assert.equal(normalizeSettings({ require_selfie: true }).require_selfie, true);
assert.equal(normalizeSettings(null).require_selfie, false);

// --- decodeSelfie -----------------------------------------------------------
// This is a trust boundary: the bytes come from a browser and are served back
// to a manager, so anything that is not actually a JPEG must not get through.
const jpegBytes = Buffer.concat([Buffer.from([0xff, 0xd8, 0xff, 0xe0]), Buffer.alloc(64, 7)]);
const asDataUrl = (buf) => `data:image/jpeg;base64,${buf.toString('base64')}`;

const decoded = decodeSelfie(asDataUrl(jpegBytes));
assert.ok(Buffer.isBuffer(decoded), 'a real JPEG data URL decodes');
assert.equal(decoded.length, jpegBytes.length);
assert.deepEqual(decoded, jpegBytes, 'bytes survive the round trip unmodified');

// Nothing else is accepted, and none of these may throw.
assert.equal(decodeSelfie(null), null);
assert.equal(decodeSelfie(''), null);
assert.equal(decodeSelfie(undefined), null);
assert.equal(decodeSelfie('not a data url'), null);
assert.equal(decodeSelfie({}), null);
// A PNG relabelled as JPEG: the declared type says image/jpeg, the magic bytes
// do not. The magic bytes win.
assert.equal(
  decodeSelfie(asDataUrl(Buffer.from([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a]))),
  null,
  'a PNG mislabelled as JPEG is rejected'
);
// Same for markup, which is the one that would actually matter if a browser
// ever sniffed the response.
assert.equal(decodeSelfie(asDataUrl(Buffer.from('<svg onload=alert(1)>'))), null);
// Real content types we do not accept, so nothing slips in on the header alone.
assert.equal(decodeSelfie(`data:image/png;base64,${jpegBytes.toString('base64')}`), null);
assert.equal(decodeSelfie(`data:text/html;base64,${jpegBytes.toString('base64')}`), null);
// Too short to carry a marker.
assert.equal(decodeSelfie(asDataUrl(Buffer.from([0xff, 0xd8]))), null);
// Over the cap. Rejected on the encoded length, before anything is allocated.
assert.equal(
  decodeSelfie(asDataUrl(Buffer.concat([Buffer.from([0xff, 0xd8, 0xff]), Buffer.alloc(301 * 1024, 1)]))),
  null,
  '300KB cap holds'
);
// Just under the cap still passes, so the cap is not off by a factor.
assert.ok(
  decodeSelfie(asDataUrl(Buffer.concat([Buffer.from([0xff, 0xd8, 0xff]), Buffer.alloc(250 * 1024, 1)]))),
  '250KB is accepted'
);
