...
export const auth = betterAuth({
  ...
  plugins: [nextCookies(), getDashPlugin(), admin()],
  databaseHooks: {
    user: {
      create: {
        after: async (user) => {
          try {
            await sql(
              `INSERT INTO stock_app_users (name, email, role, status, can_manage_users, can_approve_changes, can_view_dashboard, created_by)
               VALUES ($1, $2, 'salesperson', 'active', FALSE, FALSE, TRUE, 'self-signup')
               ON CONFLICT (email) DO NOTHING`,
              [user.name || user.email, user.email]
            );
          } catch {
            // Non-fatal: stock_app_users row will be created by admin later
          }
        },
      },
    },
