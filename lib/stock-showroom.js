 ...
 *
 * Two sets of column names come back: the stock_items rollup and the
 * per-branch stock_showroom_stock equivalents.
 */
export function resolveShowroomMove({ move: moveKey, unitOfMeasure, qty }) {
  ...
  return {
    ...
    // The company-wide rollup on stock_items...
    ...
    // ...and the same two on stock_showroom_stock, which is per branch. Both are
    // written together so the rollup always equals the sum of the branches.
    branchShowroomColumn: isStone ? 'sqft' : 'whole_qty',
    branchInstalledColumn: isStone ? 'installed_sqft' : 'installed_whole_qty',
