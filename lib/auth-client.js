...
export function useAuthUser() {
  ...
  const [lastResolvedUser, setLastResolvedUser] = useState(null);
  ...
  const resolvedSessionUser = session?.user || null;
  const fallbackSessionUser = fallbackSession?.user || null;
  const resolvedUser = resolvedSessionUser || fallbackSessionUser;

  useEffect(() => {
    const isFallbackPending = shouldUseFallbackSession && fallbackSession === null;
    const isResolving = (isPending && !timedOut) || fallbackLoading || isFallbackPending;
  ...
  }, [fallbackLoading, isPending, resolvedUser, timedOut, shouldUseFallbackSession, fallbackSession]);

  const isFallbackPending = shouldUseFallbackSession && fallbackSession === null;
  const isResolving = (isPending && !timedOut) || fallbackLoading || isFallbackPending;
  const isLoading = Boolean(!hasResolvedSessionOnce && isResolving);
  const user = resolvedUser || (isResolving && hasResolvedSessionOnce ? lastResolvedUser : null);
