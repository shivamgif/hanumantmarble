...
const invoiceFormat = (val) => !val || /^[A-Z0-9/-]+$/.test(val)
const invoiceFormatMsg = "Only A-Z, 0-9, / and - allowed"

// Deliveries very occasionally arrive with no transport paperwork at all — no
// transporter, no plate, no driver. Those still need logging, so the three
// fields are optional at the type level and required by this refinement unless
// the user explicitly ticks "No transporter details", which demands a written
// reason in its place. Shared by all three arrival forms: tile, bag and stone
// disagreed on which of the three were mandatory, for no reason anyone recorded.
const TRANSPORTER_FIELDS = [
  ["transporterName", "transporter"],
  ["truckLicensePlate", "truck number"],
  ["driverName", "driver name"],
]

export function refineTransporter(form, ctx) {
  if (form.transporterUnknown) {
    if (!String(form.notes || "").trim()) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "Explain why transporter details are unavailable.",
        path: ["notes"],
      })
    }
    return
  ...
  for (const [path, label] of TRANSPORTER_FIELDS) {
    if (!String(form[path] || "").trim()) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: `Enter the ${label}, or tick "No transporter details".`,
        path: [path],
      })
    }
  }
...
export const arrivalItemSchema = z.object({
  ...
  discountAmount: numericText.optional().default(""),
...
export const dispatchItemSchema = z.object({
  itemCategory: z.enum(["tile", "bag", "stone"]).default("tile"),
  itemId: z.string().min(1, "Select a stock item"),
  loadedWholeQty: numericText,
  loadedBrokenQty: numericText.optional().default(""),
  fromBroken: z.boolean().optional().default(false),
  sellUnit: z.enum(["box", "piece", "bag", "sqft"]).default("box"),
  ratePerUnit: numericText.optional().default(""),
  notes: z.string().optional().default(""),
  ...
  // Stone fields — fractional square feet, see lib/stock-sqft.js
  qtySqft: numericText.optional().default(""),
  returnQtySqft: numericText.optional().default(""),
}).superRefine((item, ctx) => {
  const isBag = item.itemCategory === "bag"
  const isStone = item.itemCategory === "stone"
  if (isStone) {
    if (Number(item.qtySqft || 0) <= 0) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Enter quantity in sqft.", path: ["qtySqft"] })
    }
  } else if (isBag) {
    if (Number(item.qtyBags || 0) <= 0) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Enter quantity in bags.", path: ["qtyBags"] })
    }
  } else if (item.fromBroken) {
    if (Number(item.loadedBrokenQty || 0) <= 0) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Enter broken quantity sold.", path: ["loadedBrokenQty"] })
    }
  } else {
...
export const arrivalFormSchema = z.object({
  ...
  truckLicensePlate: optionalText,
  driverName: optionalText,
  invoiceNumber: requiredText.refine(invoiceFormat, { message: invoiceFormatMsg }),
  ...
  transporterUnknown: z.boolean().default(false),
  // Truck trip this invoice rides on; "" means a delivery of its own.
  tripId: z.union([z.string(), z.number(), z.null(), z.undefined()]).transform((v) => (v == null ? "" : String(v))),
...
export const bagDispatchFormSchema = z.object({
  ...
  invoiceNumber: z.string().trim().min(1, "Invoice number is required").refine(invoiceFormat, { message: invoiceFormatMsg }),
  salespersonName: z.string().optional().default(""),
  dispatchDate: z.string().min(1, "Invoice date is required"),
  transportCost: numericText,
  laborCost: numericText,
  notes: z.string().optional().default(""),
...
// ---- Stone (Kota etc) arrivals: quantity is total sqft, fractional. GST is 5%.
// Stone DISPATCH needs no schema of its own — dispatchItemSchema above handles it
// via itemCategory === 'stone', because dispatches are one form with mixed rows. ----

export const stoneArrivalItemSchema = z.object({
  id: z.number().optional(),
  itemCategory: z.string().default("stone"),
  itemId: z.string().optional().default(""),
  itemName: requiredText,
  brandName: requiredText,
  typeName: requiredText,
  // Slab size is recorded per consignment for the record only — it changes
  // between deliveries, so it is NOT used to derive stock. qtySqft is the truth.
  sizeLabel: z.string().optional().default(""),
  qtySqft: requiredNumericText.refine((v) => Number(v) > 0, { message: "Enter quantity in sqft." }),
  ratePerSqft: requiredNumericText.refine((v) => Number(v) > 0, { message: "Enter a rate per sqft." }),
...
export const stoneArrivalFormSchema = z.object({
  supplierName: requiredText,
  truckLicensePlate: optionalText,
  driverName: optionalText,
  invoiceNumber: requiredText.refine(invoiceFormat, { message: invoiceFormatMsg }),
  ...
  handlingCostPercent: numericText,
  fuelCostPercent: numericText,
  // Stone is taxed at 5%, tiles at 18%. Still editable — this is only the default.
  gstPercent: numericText,
  freightWeightKg: numericText,
  discountAmount: numericText.optional().default(""),
  notes: z.string().optional().default(""),
  items: z.array(stoneArrivalItemSchema).min(1, "Add at least one stone item."),
}).superRefine(refineTransporter)
...
export const dispatchFormSchema = z.object({
  ...
  invoiceNumber: z.string().trim().min(1, "Invoice number is required")
    .refine(invoiceFormat, { message: invoiceFormatMsg }),
  salespersonName: z.string().optional().default(""),
  salespersonUserId: z.string().trim().min(1, "Select a salesperson from the list"),
  dispatchDate: z.string().min(1, "Invoice date is required"),
  transportCost: numericText,
  laborCost: numericText,
  notes: z.string().optional().default(""),
