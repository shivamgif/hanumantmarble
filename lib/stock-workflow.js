...
let cachedDatabaseAvailable = globalThis._dbAvailableStatus ?? null;

export async function ensureDatabaseAvailable() {
  if (cachedDatabaseAvailable !== null) {
    return cachedDatabaseAvailable;
  }

  try {
    await sql('SELECT 1');
    cachedDatabaseAvailable = true;
    globalThis._dbAvailableStatus = true;
    return true;
  } catch (error) {
    cachedDatabaseAvailable = false;
    globalThis._dbAvailableStatus = false;
    // reset after 60s to attempt recovery if down
    setTimeout(() => { 
      cachedDatabaseAvailable = null; 
        ...
        divisionIds && divisionIds.length > 0
          ? `SELECT name, phone, email, role
             ...
             AND (role IN ('admin','manager','stock_maintainer')
                  OR (role = 'salesperson' AND division_id = ANY($1)))`
          : `SELECT name, phone, email, role
             FROM stock_app_users
             WHERE status = 'active'`,
